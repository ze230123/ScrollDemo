//
//  UNavigationBar.swift
//  
//
//  Created by 张泽群 on 2022/11/28.
//
...
private var navigationBarKey = "UNavigationBarKey"

extension UIViewController {
    fileprivate var bar: UNavigationBar? {
        get {
            return withUnsafePointer(to: &navigationBarKey) { pointer in
                return objc_getAssociatedObject(self, pointer) as? UNavigationBar
            }
        }
        set {
            withUnsafePointer(to: &navigationBarKey) { pointer in
    ...
    /// 导航栏设置工具
    public var uNavigationBar: UNavigationBar {
        guard let bar = bar else {
            let bar = UNavigationBar(navigationController?.navigationBar)
            self.bar = bar
            return bar
        }
        return bar
...
/// ios 13 之前设置
fileprivate extension UNavigationBar {

    /// 自定义透明导航view
    var overlay: UIView? {
        get {
            withUnsafePointer(to: &overlayKey) { pointer in
                return objc_getAssociatedObject(self, pointer) as? UIView
            }
        }
        set {
            withUnsafePointer(to: &overlayKey) { pointer in
